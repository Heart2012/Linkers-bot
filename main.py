        ...
        for ch in CHANNELS:
            try:
                invite = await bot.create_chat_invite_link(
                    chat_id=ch["id"],
                    name=link_name,
                    creates_join_request=True  # закрита лінка з підтвердженням заявки
                )
...
# ================== Запуск бота ==================
async def main():
    await bot.delete_webhook(drop_pending_updates=True)  # удаляем вебхук перед polling
